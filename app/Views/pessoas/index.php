...
<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-4">
    <div>
        ...
        <p class="text-secondary mb-0">Cadastro, edição e vinculação de usuários.</p>
    </div>
        ...
        <form id="formPessoa">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select" name="usuario_id" id="selectUsuario">
...
<script>
    ...
    async function carregarUsuariosParaSelect() {
        try {
            const usuarios = AtendeLabApi.toList(await AtendeLabApi.get('usuarios', 'listar'));
            const select = document.getElementById('selectUsuario');
            select.innerHTML = '<option value="">Selecione um usuário...</option>' + 
                usuarios.map(u => `<option value="${u.id}">${AtendeLabApi.escape(u.nome)}</option>`).join('');
    ...
    async function carregarPessoas() {
        try {
            ...
            <td>${AtendeLabApi.escape(p.usuario_relacionado || 'Nenhum')}</td>
            ...
            <td class="text-end">
                ...
                ${p.status === 'ativo'
                    ...
                    : `<button class="btn btn-sm btn-outline-success" onclick="reativarPessoa(${Number(p.id)})">Reativar</button>`}
            </td>
        </tr>`).join('');
    ...
    async function reativarPessoa(id) {
        if (!confirm('Deseja reativar esta pessoa?')) return;
        try {
            await AtendeLabApi.post('pessoas', 'reativar', { id });
            await carregarPessoas();
        } catch (error) { AtendeLabApi.showAlert('alerta', error.message, 'danger'); }
    }
